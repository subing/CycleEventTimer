...
func (c *CircleEvent) Put(v interface{}, flag bool) {
	c.taskIn <- indata{
		value: v,
		must:  flag,
	}
...
// Pop 从环中移除
func (c *CircleEvent) Pop(v interface{}) {
	c.taskOut <- v
}

// Close 不再接收put,pop 返回当前环中所有数据
func (c *CircleEvent) Close() (ret []interface{}) {
	close(c.closed)
...
func (c *CircleEvent) put(in indata) {
